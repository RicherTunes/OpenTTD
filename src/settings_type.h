...
enum class ContinentShape : uint8_t {
	...
	Island,       ///< Single central landmass surrounded by water.
	Archipelago,  ///< Multiple island clusters.
...
enum class TerrainAlgorithm : uint8_t {
	Classic,         ///< Original TGP Perlin noise algorithm.
	ImprovedPerlin,  ///< Improved Perlin with smoother gradients.
};

/** Biome model for climate zone distribution. */
enum class BiomeModel : uint8_t {
	Classic,          ///< Original climate-based zone distribution.
	TemperatureBased, ///< Temperature gradient based on altitude and noise.
};

/** Town distribution algorithm. */
enum class TownDistribution : uint8_t {
	Random, ///< Random placement (classic behavior).
	Even,   ///< Voronoi-based even distribution.
};

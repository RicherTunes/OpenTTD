...
static const char *_frag_shader_pp_vignette[] = {
	...
	"  uv.x *= aspect;",
	"  float dist = length(uv) / aspect;  /* Normalize so radius covers short axis. */",
...
static const char *_frag_shader_pp_bicubic[] = {
	...
	"",
	"  float src_alpha = texture(source_tex, tex_coord).a;",
	"  frag_colour = vec4(clamp(result.rgb, 0.0, 1.0), src_alpha);",
	"}",
};
...
static const char *_frag_shader_pp_crt[] = {
	...
	"  float src_alpha = center.a;",
	"",
	"  /* Darken every other scanline (fixed 480 lines like a real CRT). */",
...
static const char *_frag_shader_pp_weather[] = {
	...
	"  /* Note: tex_coord Y=0 is bottom in GL, but screen Y=0 is top. */",
	"  /* Adding to uv.y moves the pattern upward in UV = downward on screen. */",
...
/** Fragment shader for animated water wave distortion.
 * Detects water pixels via blue-channel excess and applies multi-frequency
 * sine-wave displacement with a subtle brightness sparkle. Independent of
 * the screen-space water reflection pass. */
static const char *_frag_shader_pp_water_waves[] = {
	"#version 150\n",
	"uniform sampler2D source_tex;",
	"uniform vec2 texel_size;",
	...
	"  float is_water = smoothstep(0.02, 0.15, blue_excess);",
	"  if (is_water < 0.01) { frag_colour = base; return; }",
	"  float wave1 = sin(tex_coord.x * 60.0 + tex_coord.y * 30.0 + time * wave_speed * 0.04);",
	"  float wave2 = sin(tex_coord.x * 40.0 - tex_coord.y * 50.0 + time * wave_speed * 0.03);",
	"  float wave3 = sin(tex_coord.y * 80.0 + time * wave_speed * 0.05) * 0.5;",
	"  vec2 offset = vec2(wave1 + wave2, wave2 + wave3) * wave_amplitude * texel_size * is_water * 0.5;",
	"  vec4 displaced = texture(source_tex, clamp(tex_coord + offset, 0.0, 1.0));",
	/* Add slight brightness variation for sparkle */
	"  float sparkle = 0.02 * sin(tex_coord.x * 200.0 + time * wave_speed * 0.1);",
	"  vec3 result = mix(base.rgb, displaced.rgb + sparkle, is_water * 0.7);",
	"  frag_colour = vec4(clamp(result, 0.0, 1.0), base.a);",
	"}",
};
...
/** Fragment shader for seasonal vegetation colour tinting.
 * Detects vegetation pixels via green-channel excess and applies a seasonal
 * colour tint that cycles through spring, summer, autumn, and winter. */
static const char *_frag_shader_pp_seasonal[] = {
	"#version 150\n",
	"uniform sampler2D source_tex;",
	"uniform float season;",      /* 0.0=spring, 0.25=summer, 0.5=autumn, 0.75=winter */
	"uniform float intensity;",
	"in vec2 tex_coord;",
	"out vec4 frag_colour;",
	"void main() {",
	"  vec4 base = texture(source_tex, tex_coord);",
	"  float green_excess = base.g - max(base.r, base.b);",
	"  float is_vegetation = smoothstep(0.02, 0.12, green_excess);",
	"  if (is_vegetation < 0.01) { frag_colour = base; return; }",
	/* Season colour tints */
	"  vec3 spring_tint = vec3(0.6, 1.0, 0.5);",    /* Bright green */
	"  vec3 summer_tint = vec3(0.4, 0.9, 0.3);",    /* Deep green */
	"  vec3 autumn_tint = vec3(1.0, 0.6, 0.2);",    /* Orange/red */
	"  vec3 winter_tint = vec3(0.7, 0.7, 0.6);",    /* Grey/brown */
	/* Interpolate between seasons */
	"  vec3 tint;",
	"  if (season < 0.25) tint = mix(spring_tint, summer_tint, season * 4.0);",
	"  else if (season < 0.5) tint = mix(summer_tint, autumn_tint, (season - 0.25) * 4.0);",
	"  else if (season < 0.75) tint = mix(autumn_tint, winter_tint, (season - 0.5) * 4.0);",
	"  else tint = mix(winter_tint, spring_tint, (season - 0.75) * 4.0);",
	"  vec3 tinted = base.rgb * tint;",
	"  frag_colour = vec4(mix(base.rgb, tinted, is_vegetation * intensity), base.a);",
	"}",
};

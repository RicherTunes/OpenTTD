	...
	std::string GetWidgetString(WidgetID widget, StringID stringid) const override
	{
		switch (widget) {
			...
			case WID_GO_VIDEO_UPSCALE_DROPDOWN: {
				...
				return GetString(_upscale_names[Clamp<uint8_t>(_video_upscale_mode, 0, 4)]);
			}
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		switch (widget) {
			...
			case WID_GO_VIDEO_RENDER_SCALE:
				DrawSliderWidget(r, GAME_OPTIONS_BACKGROUND, GAME_OPTIONS_BUTTON, TC_BLACK, 25, 200, 16, _video_render_scale, GpuPercentMarkFunc);
				break;
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		...
		switch (widget) {
			...
			case WID_GO_VIDEO_RENDER_SCALE:
				...
				if (ClickSliderWidget(this->GetWidget<NWidgetBase>(widget)->GetCurrentRect(), pt, 25, 200, _ctrl_pressed ? 0 : 16, _video_render_scale)) {
...
#undef GPU_SLIDER_CLICK
	...
	void OnDropdownSelect(WidgetID widget, int index, int) override
	{
		switch (widget) {
			...
			case WID_GO_VIDEO_UPSCALE_DROPDOWN:
				_video_upscale_mode = Clamp(index, 0, 4); /* 0=None, 1=Bilinear, 2=FSR1, 3=Temporal, 4=Plugin */
				this->SetDirty();
				break;
				...
				NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_wide, 0),
					...
					NWidget(WWT_FRAME, GAME_OPTIONS_BACKGROUND), SetStringTip(STR_GAME_OPTIONS_GPU_RENDERING), SetTextStyle(GAME_OPTIONS_FRAME),
						NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_normal, 0),
							...
							EndContainer(),
							/* Night mode + collapsible sub-parameters */
							NWidget(NWID_HORIZONTAL), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0),
							...
							EndContainer(),
							NWidget(NWID_SELECTION, INVALID_COLOUR, WID_GO_VIDEO_NIGHT_PARAMS_SEL),
								NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_normal, 0),
									NWidget(NWID_HORIZONTAL), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0),
								...
								EndContainer(),
							EndContainer(),
							/* CRT filter + collapsible sub-parameters */
							NWidget(NWID_HORIZONTAL), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0),
							...
							EndContainer(),
							NWidget(NWID_SELECTION, INVALID_COLOUR, WID_GO_VIDEO_CRT_PARAMS_SEL),
								NWidget(NWID_VERTICAL), SetPIP(0, WidgetDimensions::unscaled.vsep_normal, 0),
									NWidget(NWID_HORIZONTAL), SetPIP(0, WidgetDimensions::unscaled.hsep_wide, 0),
